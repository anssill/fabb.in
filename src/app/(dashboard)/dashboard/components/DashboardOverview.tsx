...
export async function DashboardOverview({ businessId, branchId, today }: { businessId: string; branchId: string; today: string }) {
  ...
  const months = Array.from({length:12},(_,i)=>{const d=new Date(`${today.slice(0,7)}-01T12:00:00Z`);d.setUTCMonth(d.getUTCMonth()-11+i);return d.toISOString().slice(0,7)}).filter(month => bookings.some(b => b.created_at.slice(0,7) === month && b.status !== "cancelled") || payments.some(p => p.posted_at.slice(0,7) === month && ["payment","refund","reversal"].includes(p.entry_type)))
