...
export function ItemsStep({ items, setItems }: Props) {
  ...
  const handleSearch = async () => {
    if (!searchQuery || searchQuery.length < 2 || !staff?.business_id) return
    setSearching(true)
    try {
      const supabase = createClient()
      const { data } = await supabase
        .from('items')
        .select('id, name, sku, category, daily_rate, cover_image_url, item_variants(id, size, colour, available_stock)')
  ...
  const addItem = (item: SearchResult, variant: SearchResult['item_variants'][0]) => {
    ...
    if (exists) {
      toast('This variant is already added')
      return
    }
    setItems([
      ...items,
              ...
              <div key={item.id} className="border rounded-md p-3">
                <div className="flex items-center gap-3 mb-2">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-slate-500">{item.category} · ₹{item.daily_rate}/day</p>
                  </div>
                </div>
                <div className="flex flex-wrap gap-2">
                  {item.item_variants
                    ...
                    .map((v) => (
                      ...
                      >
                        {v.size}{v.colour ? ` · ${v.colour}` : ''} ({v.available_stock})
                      </Button>
                    ))}
                  {(!item.item_variants || item.item_variants.every((v) => v.available_stock <= 0)) && (
          ...
          <div className="space-y-2">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-slate-500">
                      {item.size}{item.colour ? ` · ${item.colour}` : ''} · ₹{item.daily_rate}/day
                    </p>
                  </div>
                </div>
            ...
            <div className="text-right">
              <p className="text-sm text-slate-500">
                Subtotal: <span className="font-semibold text-slate-900">₹{items.reduce((s, i) => s + i.daily_rate * i.quantity, 0).toLocaleString('en-IN')}</span>/day
              </p>
            </div>
          </div>
        )}
...
function toast(msg: string) {
  // Use import from sonner in actual implementation
  console.log(msg)
}
