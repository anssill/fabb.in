...
export function ReceiptStep({ bookingId, customer, items, dates, pricing, payment }: Props) {
  const balanceDue = pricing.total_amount - payment.advance_amount
          ...
          <div className="space-y-2">
            ...
            {items.map((item) => (
              <div key={item.variant_id} className="flex justify-between text-sm">
                ...
                <span className="font-medium">₹{(item.daily_rate * item.quantity * rentalDays).toLocaleString('en-IN')}</span>
              </div>
            ))}
          </div>
