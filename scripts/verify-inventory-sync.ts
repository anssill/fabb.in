...
async function verifySync() {
  ...
  console.log(`Current Rate: ${item.price}`)

  // 3. Verify item values
  if (item.price === undefined || item.price === null) {
    console.warn(`[WARN] Item ${item.id} (${item.name}) has no price set!`)
  }

  // Check variant stock values
  if (item.item_variants) {
    for (const variant of item.item_variants) {
      if (variant.available_stock === undefined || variant.reserved_stock === undefined) {
        console.error(`[ERROR] Variant ${variant.id} is missing stock columns!`)
      }
    }
  }
